...
class TestCommandStoreLib(unittest.TestCase):
    ...
    def test_verify_read_sql_file_time(self):
        file_name = os.path.join(TEST_FILES_PATH, "remember.db")
    ...
    def test_readUnprocessedLinesOnly(self):
    ...
    def test_CuratedCommands_ReturnCorrectResults(self):
        self.assertEqual("git foo",
                         command_store_lib.Command.get_currated_command("    git     foo"))
        self.assertEqual(". git foo",
                         command_store_lib.Command.get_currated_command(" .   git     foo"))

    def test_CuratedZshCommands_ReturnRemovedHeaders(self):
    ...
    def test_delete_whenDoesntExists_shouldReturnNone(self):
        store = command_store_lib.SqlCommandStore(':memory:')
    ...
    def test_create_select_query_whenSingleTermSorted_ShouldReturnBasicSortQuery(self):
    ...
    def test_create_select_query_whenSingleTermStartsWith_ShouldReturnStartsWithQuery(self):
        query = command_store_lib._create_command_search_select_query(['grep'], True, False, False)
        expected = "SELECT * FROM remember WHERE full_command LIKE 'grep%'"
        self.assertEqual(expected, query)

    def test_create_select_query_whenSingleTermStartsWithAndSort_ShouldReturnBothQuery(self):
        query = command_store_lib._create_command_search_select_query(['grep'], True, True, False)
        expected = "SELECT * FROM remember WHERE full_command LIKE 'grep%' " \
                   "ORDER BY count_seen DESC, last_used DESC"
        self.assertEqual(expected, query)

    def test_create_select_query_whenSingleTermAll3_ShouldReturnAll3Query(self):
